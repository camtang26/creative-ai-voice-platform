...
:: Set terminal title
title Local Sheet Call

echo.
echo ========================================================
echo    SHEET CALL USING LOCAL SERVER
echo ========================================================
echo.
...
if "%~1"=="" (
    ...
    echo Usage: sheet-call-local.bat SPREADSHEET_ID [SHEET_NAME] [MAX_CALLS]
    exit /b 1
)
...
echo Starting sheet call with local environment...
node -r dotenv/config sheet-call.js %* dotenv_config_path=.env.local
