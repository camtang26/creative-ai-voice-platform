...
export async function registerRecordingApiRoutes(fastify, options = {}) {
  ...
  fastify.get('/api/fetch-audio/:recordingSid', async (request, reply) => { // Changed path segment
    const { recordingSid } = request.params;
    // ADDED log at the very start
    console.log(`[API Fetch Audio Handler] Route hit for recordingSid: ${recordingSid}`); // Updated log message
    request.log.info(`[API Fetch Audio] Received request for Recording SID: ${recordingSid}`); // Updated log message
    ...
    try {
      ...
      if (!recording || !recording.url) {
        request.log.warn(`[API Fetch Audio] Recording not found or URL missing for SID: ${recordingSid}`); // Updated log message
      ...
      if (!accountSid || !authToken) {
        request.log.error('[API Fetch Audio] Missing Twilio credentials for download'); // Updated log message
      ...
      request.log.info(`[API Fetch Audio] Fetching audio from Twilio URL: ${twilioUrl}`); // Updated log message
      ...
      if (!response.ok) {
        request.log.error(`[API Fetch Audio] Failed to fetch audio from Twilio. Status: ${response.status} ${response.statusText}`); // Updated log message
      ...
      request.log.info(`[API Fetch Audio] Sending stream via reply.send() for ${recordingSid}`); // Updated log message
      return reply.send(response.body);
      
    } catch (error) {
      request.log.error(`[API Fetch Audio] Error processing download for ${recordingSid}:`, error); // Updated log message
      if (!reply.sent) {
        reply.code(500).send({
          success: false,
