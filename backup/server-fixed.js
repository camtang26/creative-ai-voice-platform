...
callHandler.setActiveCallsReference(activeCalls);
callHandler.setTwilioClientReference(twilioClient);
...
callHandler.startCallMonitoringHeartbeat(5000); // Check every 5 seconds
...
server.post('/amd-status-callback', async (request, reply) => {
  try {
    ...
    // Process machine detection results using the call handler
    callHandler.processMachineDetection(amdData);
...
server.post('/call-status-callback', async (request, reply) => {
  try {
    ...
    callHandler.updateCallActivity(CallSid);
...
server.post('/api/calls/:callSid/terminate', async (request, reply) => {
  ...
  try {
    ...
    // Use the call handler to terminate the call
    const result = await callHandler.terminateCall(callSid, { force, reason });
...
const start = async () => {
  try {
    ...
    callHandler.stopCallMonitoringHeartbeat();
    process.exit(1);
  }
};
